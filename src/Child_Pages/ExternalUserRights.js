...
export default function ExternalUserRights() {
  ...
  const fetchUserRights = async () => {
    if (userId && userNameDropdownValue) {
      try {
        const response = await axios.get(`http://${ipaddress}externaluserrights/getuser/${userNameDropdownValue}/${companyid}/${branchId}`);
  ...
  const handleSaveUserRights = () => {
    ...
    console.log(userId2);
    axios
      .post(`http://${ipaddress}externaluserrights/insert/${userId2}`, newUserRights)
      .then(() => {
        toast.success("External User Rights Saved successfully!", "success");
        fetchUserRights();
        fetchStatus();

      })
      .catch((error) => {
        toast.error("Error While Saving External User Rights !", "error");
      });
  };
  ...
  const handleSubmitUserRights = () => {
    ...
    axios
      .put(`http://${ipaddress}externaluserrights/update/${userId}`, updatedUserRights)
      .then(() => {
        toast.success("External User Rights Submitted successfully!", "success");
        fetchUserRights();
        fetchStatus();
      
      })
      .catch((error) => {
        toast.error("Error While Submitting External User Rights !", "error");
      });
  };
  ...
  useEffect(() => {
    if (selectbranch !== '') {
      axios
        .get(`http://${ipaddress}externaluserrights/u/${selectbranch}/${companyid}/${branchId}`)
        .then((response) => {
  ...
  const fetchStatus = async () => {
    try {
      const response = await axios.get(`http://${ipaddress}externaluserrights/status/${targetUser}`);
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://${ipaddress}externaluserrights/get-user/${getapprove}/${companyid}/${branchId}`)
      .then((response) => {
  ...
  useEffect(() => {
    axios.get(`http://${ipaddress}externaluserrights/get-user/${getcreated}/${companyid}/${branchId}`)
      .then((response) => {
  ...
  useEffect(() => {
    axios.get(`http://${ipaddress}externaluserrights/get-user/${getapprove}/${companyid}/${branchId}`)
      .then((response) => {
...
<FormGroup>
  ...
  <Input
    ...
    onChange={(e) => {
      ...
      handleUserId(e);
      handlegetUser(e);
      setUserNameDropdownValue(e.target.value);
      handleuserid(e);
      handletargetuser(e);
      setActivePageParent(1);
      setActivePageChild(1);
...
<Col sm={4} style={{ marginTop: '30px' }}>
           ...
           <Button
             ...
             disabled={!selectedUsername.length>0}
           >
              ...
              {Array.isArray(statuslist) && statuslist.length > 0 ? (
                <FormGroup>
                  ...
                  <Input
                    ...
                    name="branchname"
                    id="branchname"
                    value={getstatus === 'A' ? 'Approved' : getstatus === 'N' ? 'New' : getstatus === 'E' ? 'Edited' : getstatus}
                    readOnly
                  />
