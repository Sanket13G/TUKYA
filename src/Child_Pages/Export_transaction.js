...
function Export_transaction(props) {
  ...
  const fetchimportData = async (startDate, DGDC_Status) => {
    ...
    try {
      ...
      const response = await axios.get(`http://${ipaddress}exportshb/findexportTransactionData`, { params });
  ...
  const handlePrint = async (type) => {
    ...
    try {
      const response = await axios.get(`http://${ipaddress}exportshb/exportTransactionPrint`, { params });
      ...
      if (type === 'PDF') {
        if (response.data === 'not generated') {
          ...
          downloadLink.download = 'Export Transaction.pdf'; // Set the filename for the downloaded PDF
  ...
  const handleXLSDownload = async () => {
    ...
    try {
      const response = await axios.get(`http://${ipaddress}exportshb/exportTransactionXLSDownload`, {
        params,
              ...
              <Table className="table table-bordered text-center custom-table mt-3">
                    <thead>
                      <tr>
                        ...
                        <th style={{ backgroundColor: "#BADDDA" }}>SB Date</th>
                        ...
                        <th style={{ backgroundColor: "#BADDDA" }}>Package Count</th>
                    ...
                    <tbody>
                      ...
                      {currentItems.map((currentItems, index) =>
                        ...
                        <tr className="text-center dynamic-row-width">
                          ...
                          <td>{formatDate(currentItems[3])}</td>
                          <td>{currentItems[4]}</td>
                          <td>{currentItems[5]}</td>
                          <td>{currentItems[6] + ' ' + currentItems[7]}</td>
...
export default Export_transaction;
