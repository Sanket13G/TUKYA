...
const ExternalPartyConfirmationPage = () => {
  ...
  const generateAndDownloadPDF = () => {
    ...
    // Set initial y-coordinate for printing
    let y = 10;

    // Add "DGDC" in bigger font, brown colored, centered, and bold
    pdf.setTextColor(139, 69, 19); // Brown color
    pdf.setFontSize(20); // Adjust font size as needed
    pdf.setFont('bold'); // Set the font to bold
    const dgdcText = 'DGDC';
    const dgdcTextWidth = pdf.getStringUnitWidth(dgdcText) * pdf.internal.getFontSize() / pdf.internal.scaleFactor;
    const dgdcX = (pdf.internal.pageSize.width - dgdcTextWidth) / 2;
    pdf.text(dgdcText, dgdcX, y);
    pdf.setFont('normal'); // Reset the font style
    pdf.setTextColor(0, 0, 0); // Reset text color
    pdf.setFontSize(14); // Reset font size

    // Add a small vertical spacing between "DGDC" and the next text
    y += 5;

    // Center align the header text and reduce font size
    pdf.setFontSize(10); // Adjust font size as needed
    const headerText = 'DIAMONDS AND GEM DEVELOPMENT CORPORATION LTD.';
    const headerTextWidth = pdf.getStringUnitWidth(headerText) * pdf.internal.getFontSize() / pdf.internal.scaleFactor;
    const centerX = (pdf.internal.pageSize.width - headerTextWidth) / 2;
    pdf.text(headerText, centerX, y);
    y += 5; // Reduce vertical padding

    // Reduce font size of the description text
    pdf.setFontSize(8); // Adjust font size as needed
    const descriptionText = 'Custodian of Precious Cargo at SEEPZ-SEZ, Mumbai';
    const descriptionTextWidth = pdf.getStringUnitWidth(descriptionText) * pdf.internal.getFontSize() / pdf.internal.scaleFactor;
    const descriptionX = (pdf.internal.pageSize.width - descriptionTextWidth) / 2;
    pdf.text(descriptionText, descriptionX, y);
    y += 10; // Adjust space after the description

    // Add a horizontal line below the header
    pdf.setLineWidth(0.5);
    pdf.line(10, y, pdf.internal.pageSize.width - 10, y);
    y += 10; // Adjust space after the line

    // Exclude specific rows
    const excludedRows = ['companyId', 'branchId', 'createdBy', 'createdDate', 'editedBy', 'editedDate', 'approvedBy', 'approvedDate', 'status', 'lastInVoiceNo', 'lastInVoiceDate', 'correction'];
    const filteredData = Object.entries(editedParty).filter(([key]) => !excludedRows.includes(key));

    // Loop through each party detail and add it to the PDF table
    const columns = ['Field', 'Value'];
    const data = filteredData.map(([key, value]) => [formatFieldName(key), value]);
    ...
    pdf.autoTable({
     ...
     // Add "Thanks & Regards, DGDC eCustodian Team" above the confidentiality note
     const regardsText = 'Thanks & Regards, DGDC eCustodian Team';
     const regardsTextWidth = pdf.getStringUnitWidth(regardsText) * pdf.internal.getFontSize() / pdf.internal.scaleFactor;
     const regardsX = (pdf.internal.pageSize.width - regardsTextWidth) / 2;
     const regardsY = pdf.internal.pageSize.height - 16; // Adjust the vertical position
     pdf.text(regardsText, regardsX, regardsY);

    // Add note in the center at the footer
    const noteText = 'This document is confidential. Do not share it with anyone. ';
    const noteTextWidth = pdf.getStringUnitWidth(noteText) * pdf.internal.getFontSize() / pdf.internal.scaleFactor;
    const noteX = (pdf.internal.pageSize.width - noteTextWidth) / 2;
    const noteY = pdf.internal.pageSize.height - 10;
            ...
            <Row>
              ...
              <Col md={4} className="text-center">
                ...
                <button
                  ...
                  style={{ marginRight: "10px", marginTop: "30px" }}
                >
                  <i className="bi bi-cloud-arrow-down" style={{ marginRight: "10px" }}></i>
                  PDF
                </button>
              </Col>
...
export default ExternalPartyConfirmationPage;